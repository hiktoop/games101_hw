...
// orig: light's origin point
// dir: ligtht's direction
// tnear: time they meat
// u, v: barycenter proportion
...
{
    // return false if it's parallel
    if(dotProduct(dir, crossProduct(v1 - v0, v2 - v1)) == 0) return false;

    // solve O + tD == (1 - u - v)v0 + uv1 + vv2
    auto E1 = v1 - v0;
    auto E2 = v2 - v0;
    auto S = orig - v0;
    auto S1 = crossProduct(dir, E2);
    auto S2 = crossProduct(S, E1);
    float denominator = 1.0 / dotProduct(S1, E1);
    tnear = dotProduct(S2, E2) * denominator;
    u = dotProduct(S1, S) * denominator;
    v = dotProduct(S2, dir) * denominator;
    return tnear >= 0 && 0 <= u && u < 1.0 && 0 <= v && v < 1.0;
}
...
public:
                   ...
                   float& tnear, uint32_t& index, Vector2f& uv) const override
    {
        ...
        for (uint32_t k = 0; k < numTriangles; ++k)
        {
            ...
            float t, u, v;
            if (rayTriangleIntersect(v0, v1, v2, orig, dir, t, u, v) && t < tnear) // the closest point
            {
                tnear = t;
                uv.x = u;
                uv.y = v;
